...
**A config change only reaches the pods when `ConfigRevision` is enabled.** Set `GenericReconcilerConfig.ConfigRevision: reconciler.ConfigRevisionEnabled` and the framework stamps a digest of the rendered role group ConfigMap onto the StatefulSet's **pod template** as `config.kubedoop.dev/revision` (`reconciler.AnnotationConfigRevision`), between the ConfigMap apply and the StatefulSet apply. Changing a pod-template annotation is what makes the StatefulSet controller roll the pods; without it a `configOverrides` edit updates the ConfigMap, the pod template stays byte-identical, no rollout happens, and the operator reports `ReconcileComplete=True` while the processes serve the previous configuration. (The mounted files *do* eventually refresh, which is worse: the next unrelated restart splits the role group across two configurations.) The digest covers `Data` and `BinaryData` with sorted keys and length-prefixed fields — determinism is load-bearing, since an order-dependent digest would roll the pods on every reconcile forever, and length-prefixing stops `{"ab":"c"}` and `{"a":"bc"}` colliding. It is **opt-in** (`ConfigRevisionDisabled` is the zero value) only because enabling it rolls every pod of every managed cluster exactly once as the annotation first appears; the default is expected to flip in a future minor. The framework hashes only the ConfigMap it owns — a product mounting its own ConfigMap or Secret extends the annotation itself on the built StatefulSet.
