...
- **Role（角色）**
  - 表示 Product 内的逻辑功能组件（如 HDFS 中的 NameNode 或 DataNode）。它作为 RoleGroups 的模板和分组机制，定义了由其定义继承的共享配置（Config Overrides、共享日志设置）。
...
- **Overrides（覆盖配置）**
  - 一种分层配置机制，允许精确自定义生成的资源。它支持覆盖配置文件（如 XML/Properties）、环境变量、CLI 参数、JVM 参数和 Pod 属性（通过 PodTemplateSpec）。RoleGroup 覆盖配置继承自 Role 覆盖配置并优先于后者。
...
- **业务接口**：
    ...
    - `RoleConfigExtender`：配置扩展器接口，定义解析和合并差异化配置的逻辑。

- **扩展接口**：
    - `ClusterExtension/RoleExtension`：扩展点接口，定义调和前后的自定义逻辑。
    - `ExtensionRegistry`：扩展注册表，管理所有扩展的注册、排序和执行。
...
- **核心组件**：
    - `ClusterReconciler`：集群调和器，核心调和流程的入口点，包括 role 遍历、扩展点执行和孤儿资源清理。
...
- **实现要点**：
    - CR 结构体实现 `ClusterInterface`/`RoleInterface` 接口。
...
- **通用配置扩展器**：`RoleConfigExtender[ExtConfig any]`，约束差异化配置类型，确保类型合法性。
...
- **执行顺序**：扩展按注册顺序执行。SDK 保留注册顺序以确保确定性行为。如果需要自定义排序，请使用显式优先级值。
...
调和器遍历扩展注册表中的扩展，按注册顺序执行，支持配置"扩展失败时中断流程"以适应不同的容错需求。
...
- **精确的可观测性**：为用户提供"服务视图"而非仅仅是"Pod 视图"。
- **自动化支持**：使上层工具能够准确知道集群何时真正可操作。
...
- **`RoleConfigExtender`**：定义用于自定义配置合并的配置扩展点。
...
- **ClusterReconciler**：将调和工作流（PreReconcile → Reconcile → PostReconcile）定义为固定模板。
...
2. 实现 `ClusterInterface`/`RoleInterface` 接口以适配 SDK 调和流程。
