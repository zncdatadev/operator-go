...
- **Slice 类型(CLI 参数)**:**替换(Replace)**(默认)或**追加(Append)**。
  - **Replace**:高层切片完全替换低层切片。
  - **Append**:高层项目追加到低层切片之后。
- **PodTemplate(`podOverrides`)**:Kubernetes **Strategic Merge Patch**,逐层叠加,允许对 Pod 字段细粒度覆盖(如保持卷挂载的同时更改容器镜像)。
...
- **MutatingWebhook**：
    ...
    - **特定逻辑**：产品侧实现 `ProductDefaulter` 接口为**强类型 Spec 字段**填充产品特定默认值（如 HDFS Namenode 堆大小、默认端口）。这些是*默认值*——入场期持久化进 Spec 的静态回退。
...
- **产品日志引擎**（`pkg/productlogging`）：独立于上述配置格式适配器的、与产品无关的专用日志引擎。
  ...
  - **声明**：产品通过 `ContainerLogging`（容器、框架、pattern）声明每容器日志；框架拥有 Vector source 所 glob 的日志文件名约定（`<container>.stdout.log`），使生产者与消费者不会漂移。
...
- **标准实现**：
  - `VectorSidecarProvider`（位于 `pkg/vector/`）：**共享日志管道的唯一所有者**。它创建有大小限制的共享日志 `emptyDir`，以读写方式挂载到声明的生产者容器（产品在此写入日志文件），以只读方式挂载到 Vector agent 容器，并注入 agent。配置生成（`RenderVectorConfig`）与聚合器发现（`DiscoverAggregatorAddress`）是同包内独立的纯函数。
...
- 供给器：`ListenerProvisioner`（声明式 CSI 监听器卷注册，提供 `RegisterVolume()`、`Volumes()`/`VolumeMounts()`、`AutoInject()` 方法；Service 由 `listener-operator` 创建，而非 SDK）
