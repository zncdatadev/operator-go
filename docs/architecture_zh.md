...
- **`ProductConfig`** 适合**产品内禀及派生的配置文件内容**——如按实际资源拼出的 ZooKeeper 连接串、按 Pod 序号生成的 quorum 列表、按 rolegroup 资源算出的 JVM 堆。它是*配置生成,而非默认*:在 reconcile 时计算(而非入场期焊进 Spec),使算子升级能为既有集群**重算**配置,且派生自可变状态的值始终新鲜。它作为最低合并层注入(§2.5),用户覆盖仍然胜出。
  ...
  **重算不等于送达。** 配置文件内容的变更只会让 role group 的 ConfigMap 收敛,仅此而已:Pod 模板没有变化,因而不会触发滚动,而这些产品不会在运行时重读配置文件。重启 Pod 是平台的职责,不是本 SDK 的——`commons-operator` 的 restarter 监视带 `restarter.kubedoop.dev/enable=true` 标签的工作负载,当以卷形式挂载的 ConfigMap 或 Secret 发生变化时,它会改写 Pod 模板,由工作负载控制器完成滚动。SDK 刻意不重造这套机制:自造只能覆盖框架自己拥有的那个 ConfigMap(覆盖不到挂载的 Secret、产品自己的 ConfigMap、以及证书到期),并且会让同一个意图产生两种互相竞争的表达。因此给工作负载打标签是一项部署决策;不打标签时,配置文件的变更要等到下一次重启(无论那是什么时候)才会到达运行中的进程。
