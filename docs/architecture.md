...
- **Core Components**:
    ...
    - `StatefulSetBuilder` / `DeploymentBuilder`: Workload builders, generating the K8s workload (and, with the Service builders, the Services) corresponding to role groups. Both build the pod template through one shared internal implementation (§4.1.5).
...
### 4.1.6 Workload Kinds (StatefulSet / Deployment)

**A role's role groups are built as one of two workload kinds.** `BaseRoleGroupHandler.RoleWorkloadKinds` (set via `SetRoleWorkloadKind`) maps role names to a `reconciler.WorkloadKind` — `WorkloadKindStatefulSet` (the default; a role never configured builds byte-identical output to what it always built) or `WorkloadKindDeployment`, for roles whose pods are interchangeable (a stateless UI, an API server in front of a stateful backend) and for whom a StatefulSet's per-pod identity machinery is pure cost.

The kind is a property of the *role* — of the process it runs, never of one group's sizing — and it is reconcile-invariant: a product decides it when the operator is written, the way it decides ports and container names. It therefore lives on the **handler**, not on `RoleGroupBuildContext` (the §4.1.4 split): the build context carries the inputs that depend on which cluster is being built, and the workload kind depends on none of them. Switching a live role group's kind is a **migration, and the framework enforces that**. It is not an immutable-field conflict — a StatefulSet and a Deployment of the same name are different resources, and nothing in Kubernetes objects to both existing. That is exactly the hazard: the apply path fills only the slot the handler declares, so flipping the kind would create the new workload and simply stop mentioning the old one, which keeps running. Both carry the role group's selector labels, so the client Service ends up with twice the endpoints, half of them pods of a template that is never updated again; health reads only the declared kind and reports `Available=True` throughout; and orphan discovery skips every role group the spec still declares, so nothing ever reclaims it. `checkNoForeignWorkload` therefore reads the other kind under the role group's name before applying and fails the role group with a `*ValidationError` naming the remedy. Auto-deleting the old workload was the alternative and is rejected: the trigger is indistinguishable from a handler typo, and the remedy for a typo must not be "your pods are gone".

What changes for a `WorkloadKindDeployment` role:

| aspect | StatefulSet role (default) | Deployment role |
| --- | --- | --- |
| workload slot | `RoleGroupResources.StatefulSet` | `RoleGroupResources.Deployment` — mutually exclusive with `StatefulSet`; both non-nil fails the role group at apply time |
| headless Service | emitted (`<name>-headless`, per-pod DNS identity) | **not emitted** — a Deployment's pods have no stable identity for it to name |
| client Service | emitted when service ports are declared | same — how clients reach the role does not depend on how its pods are managed |
| data PVC | opt-in via `StorageMountPath` + storage config | none; `StorageMountPath` is **ignored** (a Deployment has no volumeClaimTemplates) |
| rollout | `podManagementPolicy` Parallel, `updateStrategy` via builder | Kubernetes default RollingUpdate (25%/25%); no explicit strategy field |
| orphan teardown | scale to zero → ordered drain → delete (§4.4) | scale to zero → delete, **no drain wait** — a ReplicaSet terminates its pods all at once, so there is no order to preserve |

Everything else is deliberately identical, enforced by construction: `builder.DeploymentBuilder` and `builder.StatefulSetBuilder` embed one shared internal pod-template implementation, so the two kinds render the same containers, volumes, probes, security contexts, `MainContainerCustomizer` timing and podOverrides semantics for identical inputs. The Deployment's `.spec.selector` is immutable exactly as the StatefulSet's is, and the apply path preserves it the same way (§5.3.3). The health manager reads the right object per role through the `WorkloadKindProvider` interface, which `BaseRoleGroupHandler` implements via `WorkloadKindFor`; a handler that does not implement it is treated as all-StatefulSet.
...
2. Get the actual role group list from **two** sources and union them:
   - the **live cluster** — the role group ConfigMaps, StatefulSets and Deployments in the CR's namespace carrying `app.kubernetes.io/instance` and `app.kubernetes.io/managed-by`, controller-owned by this CR, whose `app.kubernetes.io/component` + `app.kubernetes.io/role-group` labels reconstruct exactly the object's own name via `RoleGroupResourceName`;
...
3. Calculate orphaned role groups: `orphanedGroups = actualGroups - desiredGroups`.
   ...
   >
   > All four conditions on a live object are required. A discovery ConfigMap (§ discovery) carries the same instance/managed-by pair and the same owner reference, and a product's `RoleGroupResources.ExtraResources` may carry the handler's entire label set — only a name equal to what `RoleGroupResourceName` would produce for those labels identifies the framework's own slot. The ConfigMaps are listed alongside the workloads because the teardown deletes the workload before the ConfigMap: a pass interrupted in between leaves a ConfigMap a workload-only inventory would never see again.
   >
...
5. For each orphaned role group — roles in sorted order, so the sequence of events is reproducible across the several cycles a deletion spans — advance the deletion state machine one pass: gray-delete gate, then `PDB → StatefulSet (scale to zero → drain → delete) → Deployment (scale to zero → delete, no drain wait) → ConfigMap → Service → headless Service → metrics Service`, stopping at the first step that is still in flight. A role group holds exactly one workload kind, so of the two workload steps one always settles trivially on a `NotFound` — running both is what lets the cleaner reclaim a group without trusting a record of which kind it was.
...
- **Deletion Order** — the order only means anything because each step is **confirmed gone** before the next is issued:
    ...
    2. **StatefulSet** — the ordered drain, below — then **Deployment**, in the same workload slot: scaled to zero under `RetryOnConflict` like the StatefulSet, but deleted on the next pass **without** waiting on `.status.replicas`. The drain wait exists for the StatefulSet controller's ordered, reverse-ordinal retirement, which a stateful product relies on; a Deployment's ReplicaSet terminates its pods all at once whether it is scaled down or garbage-collected, so there is no order to preserve and waiting would only stall the rest of the teardown.
    3. **ConfigMap**.
...
- **Gray Deletion (opt-in grace period)**:
  - With `GenericReconcilerConfig.GrayDeleteGracePeriod > 0`, an orphaned role group is not deleted on first detection. The cleaner stamps `orphan.zncdata.dev/pending-deletion` (an RFC3339 timestamp) on the group's primary resources — its workload (StatefulSet or Deployment) and its ConfigMap — and defers.
...
- **Status Definition**: The SDK standardizes cluster status through Generic Conditions:
  ...
  - **Degraded**: something is wrong that the operator cannot resolve on its own — a wedged or unschedulable pod, an unreadable workload object, a failing application health check. Explicitly **not** "replicas are converging"; see §4.8.2.
...
- On the **paused path** (`reconciliationPaused: true`), the loop returns `RequeueAfter: HealthCheckInterval` like a normal successful pass. A pause freezes the *resources*, not the reporting: `Available`/`Progressing` are re-evaluated from the live workloads on every wakeup, and a pod that crash-loops during a maintenance window changes nothing in the CR and so produces no watch event of its own.
...
```
ConfigMap → HeadlessService → Service → ExtraResources → workload (StatefulSet | Deployment) → PDB → MetricsService
```
...
2. **HeadlessService**: A StatefulSet requires a `serviceName` pointing to a headless Service. Kubernetes uses it to create stable, predictable DNS entries (`pod-0.svc.ns.svc.cluster.local`) for inter-pod communication. It must exist before the StatefulSet is created. A Deployment role group (§4.1.5) skips this step entirely — its pods have no stable identity for a headless Service to name.
3. **Service** (client-facing): Created before the workload so that client endpoints are available as soon as Pods become ready. Emitted for both workload kinds — how clients reach the role does not depend on how its pods are managed.
4. **ExtraResources** (product-specific objects): Applied before the workload because they are typically pod-scheduling prerequisites — e.g. a Listener CR that the pods reference through an ephemeral CSI volume (see `RoleGroupResources.ExtraResources`). **Teardown mirrors this**: an orphaned role group's extras are deleted immediately *after* its workload, so nothing a pod might still need is reclaimed while a pod could still exist. Discovery is by the role group's identity labels plus this CR's controller owner reference, over the kinds the product declared in `SetupWithManagerOptions.ExtraOwns` — the same list that gives them watches, so the two cannot drift. Extras that carry no role group labels are undiscoverable in principle and are left to owner-reference GC.
   Between this step and the workload, the registered sidecar providers' `Validate` checks run (§4.6.2) — late enough that the ConfigMap and any extras they depend on already exist, early enough that a failure never produces a Pod.
5. **Workload**: Applied after all its dependencies (configs, DNS, extras) are in place. A role group carries exactly one workload — `RoleGroupResources.StatefulSet` or `RoleGroupResources.Deployment`, per the role's declared kind (§4.1.5); both non-nil fails the role group with a `*ValidationError` before anything is written. The StatefulSet controller then creates Pods in ordinal order; a Deployment's ReplicaSet creates them all at once.
...
Orphan cleanup uses its own order — `PDB → StatefulSet → Deployment → ConfigMap → Service → headless Service → metrics Service` (see §4.4.3) — which is **not** the exact inverse of this creation order. The two orders answer different questions: creation sequences prerequisites before dependants, while deletion removes the PDB first so it cannot block pod eviction and drops the Services last.
...
- **Typed kinds** copy their spec/data from the desired object while preserving Kubernetes immutable/allocated fields: StatefulSet `selector`, `serviceName`, `volumeClaimTemplates` and `podManagementPolicy` — and Deployment `selector` — keep their live values (changing them requires a manual delete/recreate migration); ConfigMap data is replaced wholesale (removed keys disappear).
...
- **StatefulSetBuilder** / **DeploymentBuilder**: Construct `StatefulSet` / `Deployment` resources step-by-step, handling complex configurations like volumes, containers, and affinity rules; the pod-template half is one shared implementation, so the two workload kinds cannot drift by construction (§4.1.6); regression protection for that shared code is `statefulset_builder_test.go`, which the extraction left untouched.
