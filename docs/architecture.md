...
| `affinity` | **per member** — `nodeAffinity`, `podAffinity` and `podAntiAffinity` resolve independently; an empty member, or `affinity: {}`, clears | Coarser than a leaf because a single member IS one policy: a partial edit inside `nodeSelectorTerms` produces a constraint neither author wrote (the list is an OR of ANDs). Finer than wholesale because of the third layer below — with a product default underneath, replacement means a user adding a `nodeAffinity` silently deletes the anti-affinity the product ships to spread a quorum. Clearing survives, so a single-node group can still opt out. |
...
That third layer is also why `affinity` folds **per member** rather than wholesale. With only the CR's two levels — both written by one person in one file — replacement was the simpler semantic. With a product default underneath, replacement means a user who adds a `nodeAffinity` silently deletes the anti-affinity the product ships to spread a quorum, with no event and no log line. Members are therefore resolved independently, and an empty member (`podAntiAffinity: {}`) or an empty whole (`affinity: {}`) still **clears** — the single-node development escape hatch. That clearing rule works for `affinity` and not for `resources` because the schemas differ: `affinity` is `x-kubernetes-preserve-unknown-fields`, so the API server never prunes inside it and a stored `{}` is always something the user wrote, while `resources` is structural and `cpu: {}` may be a pruning artifact.

**The product's OWN fields in that block are folded by the product**, with `FoldProductConfig[T]` — the same rules, applied to the struct a product defines by embedding `*RoleGroupConfigSpec` inline and adding its own fields beside it. The framework skips the embedded pointer so the two halves cannot fight over it, and `ValidateProductConfigType[T]()` refuses shapes that cannot be folded honestly. A field that is a single policy rather than a set of knobs opts out of leaf folding with the `kubedoop:"atomic"` struct tag.
...
Eleven causes across twelve sites inside `BaseRoleGroupHandler.BuildResources`. One produces a `*ValidationError` — a `podOverrides` mount that displaces a framework-owned one — and the rest are plain wrapped errors that the reconciler re-wraps as a `*ResourceBuildError`. All of them fail one role group, not the pass: role iteration is best-effort and sorted (§4.4).
