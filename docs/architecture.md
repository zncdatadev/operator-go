...
Every cluster's pods get an **identity**: a ServiceAccount named `ServiceAccountResourceName(kind, cluster)` — `"<lowercased kind>-<cluster>"` — created at step 0 of the reconcile, controller-owned by the CR, before any role is built. The role group merely consumes it, since `RoleGroupBuildContext.ServiceAccountName` lands on the pod template. `WorkloadRBACRules` is the other half: the **permissions** that identity carries, maintained at the same point, in the same way, from the same derived name.

**The name is derived, not configured, and that is the design.** It follows the rule §3 already states for every fixed slot — the framework owns the name of what it creates, controller-owns, garbage-collects and binds to. Nothing needs to address the ServiceAccount by a product-chosen string, and offering the choice is what produced an entire class of failures: two DIFFERENT clusters resolving to ONE ServiceAccount name — a static `ServiceAccountName` is a constant, so every CR of the product in a namespace got the same one (permanent `AlreadyOwnedError` for whichever reconciled second, and GC pulling the SA out from under the first's running pods), the identity and its permissions drifting apart when only one of two call sites was updated, and a rename leaving behind an SA nothing could find. Deriving makes all of them unrepresentable rather than documented. The Kind is in the formula because a CR name alone is not unique in a namespace, and the result is also **predictable**: an IAM trust policy naming `system:serviceaccount:<ns>:hdfscluster-prod` can be written from the formula instead of read out of an operator's source.

**Creation is unconditional** for the same reason §3.1 of `security.md` gives: audit logs should reflect the specific application identity rather than a generic `default` account. That claim was only true for products that had opted in; a ServiceAccount is pure metadata, so the switch mostly served to produce clusters running as `default` by accident.

The one legitimate need the removed name fields were being pressed into service for — reaching an **externally-managed identity** (AWS IRSA, Google Workload Identity) — is `ServiceAccountAnnotationsFunc`. Those mechanisms are an annotation on the ServiceAccount and nothing else, so neither removed field could actually deliver one: the static name made the framework create its *own* SA, and swapping the SA through `podOverrides` left the RBAC behind. That override is now rejected at build time.

`reconciler.EnsureWorkloadRBAC` stays exported for a product driving the reconciler's pieces itself rather than through `GenericReconciler`.
