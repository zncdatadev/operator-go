...
- **RoleGroup**
  - The physical unit of deployment and resource isolation under a Role. Each RoleGroup maps directly to one Kubernetes workload — a `StatefulSet` by default, or a `Deployment` for roles whose handler declares `WorkloadKindDeployment` (§4.1.5) — plus its associated Service, ConfigMap and PDB. This allows a single Role to be partitioned into multiple groups with distinct hardware specifications (CPU/Memory), replica counts, or specialized configurations (e.g., a "high-performance" DataNode group vs. a "standard" group).
...
### 4.1.5 Workload Kinds (StatefulSet / Deployment)
...
- **StatefulSetBuilder** / **DeploymentBuilder**: Construct `StatefulSet` / `Deployment` resources step-by-step, handling complex configurations like volumes, containers, and affinity rules; the pod-template half is one shared implementation, so the two workload kinds cannot drift (§4.1.5).
