...
- **`ProductConfig`** is the right place for **product-intrinsic and derived config-file content** — e.g. a ZooKeeper connection string built from the actual resources, a quorum peer list from pod ordinals, or a JVM heap sized from the role group's resources. It is *config generation, not defaulting*: computing it at reconcile time (rather than freezing it into the Spec at admission) means an operator upgrade recomputes the config for existing clusters, and values derived from mutable state stay fresh. It is injected as the lowest merge layer (§2.5), so user overrides still win.

  Recomputing the ConfigMap is only half of propagation, and for a long time the SDK did only that half: the ConfigMap converged while the pods kept running the previous configuration, because the pod template was byte-identical and nothing rolled them. Reaching the running processes requires `GenericReconcilerConfig.ConfigRevision: ConfigRevisionEnabled`, which stamps a digest of the rendered ConfigMap onto the pod template (`config.kubedoop.dev/revision`). It is opt-in for now because switching it on rolls every pod of every managed cluster exactly once, as the annotation first appears — an event a cluster owner schedules rather than inherits from an upgrade. **Design intent: this becomes the default.** Until a product enables it, "upgrade propagation" above describes the ConfigMap only.
