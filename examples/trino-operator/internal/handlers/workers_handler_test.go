...
var _ = Describe("WorkersHandler", func() {
	var (
	...
	BeforeEach(func() {
		...
		buildCtx = &reconciler.RoleGroupBuildContext{
			...
			ResourceName:     "test-cluster-workers-default",
	...
	Context("NewWorkersHandler", func() {
	...
	Context("BuildResources", func() {
		...
		It("should build ConfigMap", func() {
			...
			Expect(resources.ConfigMap.Name).To(Equal("test-cluster-workers-default-config"))
		})
		...
		It("should build HeadlessService", func() {
			...
			Expect(resources.HeadlessService.Name).To(Equal("test-cluster-workers-default-headless"))
		})

		It("should not build client Service (only headless)", func() {
			resources, err := handler.BuildResources(ctx, nil, cr, buildCtx)
			Expect(err).NotTo(HaveOccurred())
		...
		It("should build StatefulSet", func() {
			...
			Expect(resources.StatefulSet.Name).To(Equal("test-cluster-workers-default"))
		})

		It("should use default port when not specified", func() {
			resources, err := handler.BuildResources(ctx, nil, cr, buildCtx)
			Expect(err).NotTo(HaveOccurred())
			Expect(resources.HeadlessService.Spec.Ports[0].Port).To(Equal(constants.DefaultHTTPPort))
		})
		...
		It("should use custom port when specified", func() {
			...
			resources, err := handler.BuildResources(ctx, nil, cr, buildCtx)
			Expect(err).NotTo(HaveOccurred())
			Expect(resources.HeadlessService.Spec.Ports[0].Port).To(Equal(int32(9091)))
		})
		...
		It("should have worker config in ConfigMap", func() {
			resources, err := handler.BuildResources(ctx, nil, cr, buildCtx)
			Expect(err).NotTo(HaveOccurred())
			Expect(resources.ConfigMap.Data).To(HaveKey("config.properties"))
			Expect(resources.ConfigMap.Data["config.properties"]).To(ContainSubstring("coordinator=false"))
		})

		It("should have JVM config in ConfigMap", func() {
			resources, err := handler.BuildResources(ctx, nil, cr, buildCtx)
			Expect(err).NotTo(HaveOccurred())
			Expect(resources.ConfigMap.Data).To(HaveKey("jvm.config"))
		})
	})

	Context("getWorkerRoleConfigPDB", func() {
		It("should return nil when spec is nil", func() {
			result := getWorkerRoleConfigPDB(nil)
			Expect(result).To(BeNil())
		})

		It("should return nil when RoleConfig is nil", func() {
			result := getWorkerRoleConfigPDB(&trinov1alpha1.WorkersSpec{})
			Expect(result).To(BeNil())
		})
