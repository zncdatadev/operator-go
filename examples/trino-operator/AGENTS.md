...
- **Framework owns the 90%.** `TrinoRoleGroupHandler` embeds `reconciler.BaseRoleGroupHandler`, so the ConfigMap, Services, StatefulSet (with sidecars + `podOverrides` applied), and PDB are built by the SDK. The handler itself carries only `ConfigMountPath` (`/etc/trino`) — reconcile-invariant. Everything role-shaped (primary container name `trino`, per-role ports, the Log4j2 log producer) is stated by `DeclareRoles`, which implements `reconciler.RoleProvider` and is called once per pass with the cr in hand.
