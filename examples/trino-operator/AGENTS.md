...
- **Logging is fully framework-owned.** `LoggingContainers` declares only the container + framework (no output file — the framework derives `<container>.stdout.log`). When a role group enables the Vector agent, the SDK's Vector provider is the single owner of the shared log volume (creates it, RW-mounts the producer, RO-mounts the agent), and — because `TrinoCluster` implements `reconciler.VectorAggregatorProvider` (`VectorAggregatorConfigMapName()` from `spec.clusterConfig`) — the framework also generates `vector.yaml` into the ConfigMap. The product writes no Vector wiring by hand.
