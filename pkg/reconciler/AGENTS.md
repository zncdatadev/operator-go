...
19. **Workload kinds are a per-role, construction-time choice.** `SetRoleWorkloadKind(role,
    ...
    lives on the handler, not on `RoleGroupBuildContext`: it is reconcile-invariant. Switching a
    live role group's kind is a delete/recreate migration, and `checkNoForeignWorkload` **enforces**
    it: the two kinds are different resources, so nothing in Kubernetes stops both existing, and the
    apply path would create the new workload and leave the old one running with the same selector.
    The reconciler reads the other kind under the role group's name before applying and fails the
    role group with a `*ValidationError` naming the `kubectl delete` that completes the migration —
    the `-headless` Service included, since a Deployment role does not build one and nothing
    reclaims it. Only a workload this CR controller-owns counts. `RoleGroupResources.StatefulSet` and `.Deployment` are mutually
